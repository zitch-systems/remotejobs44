...
export function ToastContainer() {
  ...
  return (
    <div role="status" aria-live="polite" aria-atomic="false" className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-[500] flex flex-col-reverse gap-2 w-[min(400px,90vw)] pointer-events-none">
      {toasts.map((t) => (
        <div key={t.id} className={cn(
          ...
          <button onClick={() => removeToast(t.id)} aria-label="Dismiss notification" className="text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
      ))}
